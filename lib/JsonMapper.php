...
class JsonMapper
{
    ...
    protected function inspectProperty(ReflectionClass $rc, $name)
    {
        ...
        // <customized>
        // use underscore setters instead of camelCase convention
        // $setter = 'set' . $this->getCamelCaseName($name);
        $setter = $this->get_setter_name( $name );
        // </customized>
    ...
    private function get_setter_name( $property ) {
        if ( strlen( $property ) > 3 && $this->starts_with( $property, 'is_' ) ) {
            return 'set_' . substr( $property, 3 );
        } 
